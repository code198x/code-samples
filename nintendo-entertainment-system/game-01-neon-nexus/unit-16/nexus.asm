;──────────────────────────────────────────────────────────────
; NEON NEXUS - Unit 16 sample (Polish)
; Final pass: keeps the title → play → game over loop tight, clamps
; movement to the visible area, carries high score/pause forward, and
; adds a version tag to the title screen for the release build.
;──────────────────────────────────────────────────────────────
...
.proc play_loop
    ; Pause toggle on START (edge-triggered)
    lda controller1
    and #BTN_START
    beq @chk_pause
    lda prev_controller
...
.proc maybe_update_high
    ; compare score2:score1:score0 vs high
    lda score2
    cmp high2
    bcc @done
    bne @higher
...
score_label:.byte "YOUR SCORE ",0
hi_label:   .byte "HI ",0
ver_text:   .byte "UNIT 16 - POLISH BUILD",0
