...
; Display scores on screen using nametable updates during VBlank
;===============================================================================
...
digit_tens:       .res 1
digit_ones:       .res 1
score_changed:    .res 1  ; NEW - Flag to update nametable
...
:   BIT $2002
    ...
    STA score_p2
    LDA #1
    STA score_changed    ; Force initial score display
...
;===============================================================================
; UpdateScoreDisplay - Write score digits to nametable during VBlank
; Called from NMI when score_changed flag is set
;===============================================================================
UpdateScoreDisplay:
    ; Player 1 score at row 2, column 10 (nametable address $2000 + 2*32 + 10)
    LDA $2002           ; Reset PPU address latch
    LDA #$20
    STA $2006           ; High byte of address
    LDA #$4A            ; Row 2, col 10 = $40 + 10 = $4A
    STA $2006           ; Low byte of address

    ; Convert P1 score to tiles
    LDA score_p1
    JSR DecimalToTiles
    ...
    ; Player 2 score at row 2, column 20 (nametable address $2000 + 2*32 + 20)
    LDA #$20
    STA $2006
    LDA #$54            ; Row 2, col 20 = $40 + 20 = $54
    STA $2006

    ; Convert P2 score to tiles
    LDA score_p2
    JSR DecimalToTiles
    ...
    ; Reset PPU address to safe location
    LDA #$20
    STA $2006
    LDA #$00
    STA $2006
...
CheckScoring:
    ...
    INC score_p2
    LDA #1
    STA score_changed    ; NEW - Set flag for nametable update
    LDA score_p2
    CMP #10
    BCC @reset_ball
    LDA #0
...
;===============================================================================
; NMI - Handle VBlank, update scores if changed
;===============================================================================
NMI:
    PHA
    TXA
