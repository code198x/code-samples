; ============================================================================
; INK WAR - Unit 33: Enhanced Title Screen
; ============================================================================
; Large colourful logo using attribute blocks for visual impact.
; Professional title screen that makes a strong first impression.
;
...
; Title screen positions (enhanced large logo)
LOGO_ROW    equ     2               ; Large logo starts higher
LOGO_COL    equ     5               ; Logo area 22 columns wide
TITLE_ROW   equ     8               ; "INK WAR" text below logo
...
state_title:
            ...
            ; Play title music note
            call    play_title_music
...
; ----------------------------------------------------------------------------
; Cursor now pulses through brightness states for better visibility
...
draw_cursor:
            ...
            ; Empty cell - animate cursor colour
            call    get_cursor_anim_attr
            jr      .dc_set

.dc_p1:
            ; On P1 cell - animate with P1 base colour
            ld      a, P1_CURSOR
            call    apply_cursor_pulse
            jr      .dc_set

.dc_p2:
            ; On P2 cell - animate with P2 base colour
            ld      a, P2_CURSOR
            call    apply_cursor_pulse
...
; ----------------------------------------------------------------------------
; Get Cursor Animation Attribute
; ----------------------------------------------------------------------------
; Returns animated cursor colour based on frame counter

get_cursor_anim_attr:
            ; Use frame counter to cycle through colours
            ; Cycle: White → Cyan → White → Yellow → repeat
            ld      a, (frame_counter)
            and     %00110000           ; Use bits 4-5 (cycle every 16 frames)
            rrca
            rrca
            rrca
            ...
            ; Map to colour
            or      a
            jr      z, .gca_white       ; 0 = white
            cp      1
            jr      z, .gca_cyan        ; 1 = cyan
            cp      2
            jr      z, .gca_white2      ; 2 = white
            ; 3 = yellow

.gca_yellow:
            ld      a, %01110000        ; Yellow paper + BRIGHT
            ret
.gca_cyan:
            ld      a, %01101000        ; Cyan paper + BRIGHT
            ret
.gca_white:
.gca_white2:
            ld      a, %01111000        ; White paper + BRIGHT
            ret

; ----------------------------------------------------------------------------
; Apply Cursor Pulse
; ----------------------------------------------------------------------------
; A = base attribute, toggles BRIGHT bit based on frame counter
...
; ----------------------------------------------------------------------------
; Claim Cell (with animation)
; ----------------------------------------------------------------------------
; Flashes white before settling to player colour for satisfying feedback
...
; Brief delay for claim flash (about 3 frames)
claim_flash_delay:
            ld      bc, 6000
.cfd_loop:
            dec     bc
...
; Draw Title Screen
; ----------------------------------------------------------------------------
; Displays large colourful logo and menu options

draw_title_screen:
            ; Draw large attribute logo first
            call    draw_large_logo

            ; Draw "INK WAR" title text below logo
            ld      b, TITLE_ROW
            ld      c, TITLE_COL
            ld      hl, msg_title
            ld      e, TEXT_ATTR
...
; Draw Large Logo
; ----------------------------------------------------------------------------
; Creates a large colourful "INK WAR" logo using attribute blocks
; Logo is 22 columns × 5 rows of colourful attribute cells

draw_large_logo:
            ; Draw colourful block pattern for visual impact
            ; Row 1: Red-Yellow-Green pattern
            ld      a, LOGO_ROW
            ld      c, LOGO_COL
            ld      b, 22               ; 22 columns
            ...
            ; Row 2: Yellow-Green-Cyan pattern
            ld      a, LOGO_ROW + 1
            ld      c, LOGO_COL
            ld      b, 22
            ld      e, %01110000        ; Yellow paper + BRIGHT
            call    set_attr_range_color

            ; Row 3: Green-Cyan-Blue (centre highlight)
            ld      a, LOGO_ROW + 2
            ld      c, LOGO_COL
            ld      b, 22
            ld      e, %01100000        ; Green paper + BRIGHT
            call    set_attr_range_color

            ; Row 4: Cyan-Blue-Magenta
            ld      a, LOGO_ROW + 3
            ld      c, LOGO_COL
            ld      b, 22
            ld      e, %01101000        ; Cyan paper + BRIGHT
            call    set_attr_range_color

            ; Row 5: Blue-Magenta-Red
            ld      a, LOGO_ROW + 4
            ld      c, LOGO_COL
            ld      b, 22
            ld      e, %01001000        ; Blue paper + BRIGHT
            call    set_attr_range_color

            ret

; ----------------------------------------------------------------------------
; Set Attribute Range with Colour Variation
; ----------------------------------------------------------------------------
; A = row, C = start column, B = count, E = base attribute
; Creates subtle variation across the row

set_attr_range_color:
            push    bc
            push    de
...
; Animate Title
; ----------------------------------------------------------------------------
; Cycles title colours for eye-catching effect
...
animate_title:
            ...
            ; Default to red
.at_red:
            ld      e, %01010111        ; Red paper, white ink, BRIGHT
            jr      .at_apply
.at_yellow:
            ld      e, %01110111        ; Yellow paper, white ink, BRIGHT
            jr      .at_apply
.at_green:
            ld      e, %01100111        ; Green paper, white ink, BRIGHT
            jr      .at_apply
.at_cyan:
            ld      e, %01101111        ; Cyan paper, white ink, BRIGHT
            jr      .at_apply
.at_blue:
            ld      e, %01001111        ; Blue paper, white ink, BRIGHT
            jr      .at_apply
.at_magenta:
            ld      e, %01011111        ; Magenta paper, white ink, BRIGHT
            jr      .at_apply
.at_white:
            ld      e, %01111000        ; White paper, black ink, BRIGHT

.at_apply:
            ; Set title attributes (7 characters at TITLE_ROW, TITLE_COL)
            ld      a, TITLE_ROW
            ld      c, TITLE_COL
            ld      b, 7                ; "INK WAR" = 7 characters
            call    set_attr_range
...
; Play Title Music
; ----------------------------------------------------------------------------
; Plays one note from the title melody every N frames
...
.ptm_play_note:
            ; Reset timer (12 frames between notes = 0.24 sec)
            ld      a, 12
            ld      (music_timer), a
            ...
            ; Check for end of melody (0 = restart)
            or      a
            jr      nz, .ptm_have_note
...
.ptm_have_note:
            ; A = pitch value (delay for beeper)
            ; Play short beep
            ld      c, a            ; Save pitch
            ld      b, 8            ; Duration (very short)
...
; Title melody data (pitch values, 0 = loop)
; Higher value = lower pitch
title_melody:
            defb    30, 35, 30, 40          ; Simple rising/falling pattern
            defb    35, 30, 40, 35
            defb    30, 35, 30, 45
            defb    40, 35, 30, 25
...
msg_version:    defb    "PHASE 3 V0.1", 0
...
frame_counter:  defb    0               ; Counts frames for animations
music_index:    defb    0               ; Current note in title melody
music_timer:    defb    0               ; Countdown to next note
cursor_row:     defb    0
cursor_col:     defb    0
key_pressed:    defb    0
