; Gloaming — Unit 6: Save and Restore, demonstrated (an aside, not a step)
; Adds a bright marker in the lamplighter's path. He walks over it and it
; survives — saved on the way in, restored on the way out. The real game
; has no marker; this exists only to make save/restore visible on screen.
...
.sides:
            ...
            ; --- DEMO: a marker on the floor in his path (not in the unit proper) ---
            ; Column 11: a solid bright-yellow block — scenery worth keeping.
            ; In Unit 5 the blank-erase wiped this out; here save/restore keeps it.
            ld      hl, ROW_ATTR + 11
            ld      (hl), %01000110         ; BRIGHT, INK yellow (6)
            ld      hl, ROW_SCR + 11
            ld      b, 8
            ld      a, %11111111
.marker:
