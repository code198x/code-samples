...
; play_step — one beat of the game. For one step only, a probe:
; one attribute cell counts the frames, so the beat can be seen.
play_step:
            ld      hl, $5800 + 31  ; the HUD row's far corner
            inc     (hl)            ; one tick per beat
            ret
