; The Long Night — Unit 10: The Dawn Phrase
; Cumulative build; every step runs on its own. Narrative: the unit page.
; Motif discipline: dusk's bell, reversed — and the build is the shipped game.
...
play_step:
            ...
            cp      STATE_PLAY
            ret     nz
            ; the win check comes FIRST, before the wisp gets its move. Completing
            ; the watch holds the night this instant: a catch on the very frame the
            ; eighth lamp lights must resolve as the hold, not the fall — you
            ; finished before the dark reached you. On the fifth watch that is the
            ; difference between dawn and dawn denied.
            ld      a, (lit_count)
            cp      NUM_LAMPS
            jr      z, .watchdone
            call    draught_step
...
.pmove:
            ...
            cp      (hl)
            jr      nz, .pcommit
            ; the wisp is on the target cell — but what is the real ground beneath
            ; it? wall_at and the DARK test read the wisp's own attribute off the
            ; screen and were fooled. The truth is in the wisp's buffer: the ground
            ; it stands on. A wall or a pool there still blocks — the cold cannot
            ; reach you through the stone or the dark it is passing over.
            ld      a, (under_draught + 8)
            bit     WALL_BIT, a
            ret     nz              ; real wall under the wisp — blocked, harmless
            cp      DARK
            ret     z               ; a pool of dark under the wisp — also blocked
