; ============================================================================
; SHADOWKEEP — Unit 6, Stage 3: Victory
; ============================================================================
; The exit cell exists. This stage gives it a job.
;
; When `try_move` lands the hero on an EXIT cell, the `.walk` path branches
; into `victory` — a tiny endgame routine that turns the BORDER green and
; halts forever. No more input, no more movement; the game's first end
; state. Unit 8 will replace this halt-loop with a title → play → end →
; restart cycle, but Stage 3 here is the win state itself, sitting in one
; place ready to be wrapped.
;
; This stage also keeps the gold pickup mechanic from Stage 1. The win
; fires regardless of gold count — Stage 4 will tighten that.
; ============================================================================
...
; ----------------------------------------------------------------------------
; victory: BORDER green, halt forever. The end-of-game state for Unit 6.
; Unit 8 will replace this halt with a return to a title screen.
; ----------------------------------------------------------------------------
...
.exit_step:
            call    draw_hero                   ; show hero on the exit
            jp      victory
