...
825 REM Animate the line incrementally
830 LET ci = 4: IF pz > 2 THEN LET ci = 5
832 LET an = 10
834 LET dx = (ex - sx) / an: LET dy = (ey - sy) / an
836 LET lx = sx: LET ly = sy
838 FOR j = 1 TO an
839 LET nx = sx + dx * j: LET ny = sy + dy * j
840 PLOT INK ci; INT lx, INT ly: DRAW INK ci; INT (nx - lx), INT (ny - ly)
841 LET lx = nx: LET ly = ny
842 NEXT j
