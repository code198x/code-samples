5 REM === REJECTION ANIMATION ===
10 BORDER 0: PAPER 0: INK 7: CLS
20 DIM x(14): DIM y(14)
25 DIM f(14): DIM g(14)
26 DIM u(20)
30 LET sc = 0: LET pz = 1: LET tp = 4
...
247 GO TO 140
250 REM === select node ===
255 IF s1 = 0 THEN LET s1 = cn: BEEP 0.02, 15: GO SUB 600: GO TO 140
260 IF cn = s1 THEN GO SUB 650: LET s1 = 0: GO TO 140
265 GO SUB 650
270 GO SUB 700
275 LET s1 = 0
280 GO TO 140
...
510 RETURN
600 REM === show preview from s1 ===
605 FOR i = 1 TO nc
610 LET sk = 0
615 FOR j = 1 TO up
620 IF u(j) = i THEN LET sk = 1
625 NEXT j
630 IF sk = 1 THEN GO TO 648
635 LET tg = 0
640 IF f(i) = s1 THEN LET tg = g(i)
642 IF g(i) = s1 THEN LET tg = f(i)
644 IF tg = 0 THEN GO TO 648
646 CIRCLE INK 3; x(tg), y(tg), 5
648 NEXT i
649 RETURN
650 REM === erase preview from s1 ===
652 FOR i = 1 TO nc
654 LET sk = 0
656 FOR j = 1 TO up: IF u(j) = i THEN LET sk = 1: NEXT j
658 IF sk = 1 THEN GO TO 680
662 LET tg = 0
664 IF f(i) = s1 THEN LET tg = g(i)
666 IF g(i) = s1 THEN LET tg = f(i)
668 IF tg = 0 THEN GO TO 680
670 CIRCLE INK 0; x(tg), y(tg), 5
680 NEXT i
682 GO SUB 400
684 REM Redraw existing lines
686 FOR i = 1 TO up
688 LET ci = 4: IF pz > 2 THEN LET ci = 5
690 PLOT INK ci; x(f(u(i))), y(f(u(i))): DRAW INK ci; x(g(u(i))) - x(f(u(i))), y(g(u(i))) - y(f(u(i)))
692 NEXT i
694 GO SUB 450
696 RETURN
...
1550 REM Band 2 (cyan)
1555 DATA 5
1560 DATA 50,120, 128,140, 206,120, 90,60, 166,60
1565 DATA 5
1570 DATA 1,2, 2,3, 4,5, 1,4, 3,5
1575 DATA 6
1580 DATA 80,140, 176,140, 40,90, 216,90, 80,40, 176,40
1585 DATA 5
1590 DATA 1,2, 3,4, 5,6, 1,3, 2,4
