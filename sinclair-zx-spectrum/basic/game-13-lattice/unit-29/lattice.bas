...
247 GO TO 140
250 REM === select node ===
255 IF s1 = 0 THEN LET s1 = cn: BEEP 0.02, 15: GO SUB 480: GO TO 140
260 IF cn = s1 THEN LET s1 = 0: BEEP 0.02, 8: GO SUB 400: GO SUB 450: GO TO 140
265 GO SUB 700
270 LET s1 = 0
275 GO SUB 400: GO SUB 450
280 GO TO 140
...
405 FOR a = 1 TO nn
410 LET i = 7
414 LET nc2 = 0: LET dc2 = 0
416 FOR j = 1 TO nc
417 IF f(j) = a OR g(j) = a THEN LET nc2 = nc2 + 1
418 NEXT j
419 FOR j = 1 TO up
420 IF f(u(j)) = a OR g(u(j)) = a THEN LET dc2 = dc2 + 1
421 NEXT j
422 IF nc2 > 0 AND dc2 = nc2 THEN LET i = 4: IF pz > 4 THEN LET i = 5
425 GO SUB 300
430 NEXT a
435 RETURN
450 REM === draw cursor ===
...
475 RETURN
480 REM === highlight connected neighbours of s1 ===
482 FOR i = 1 TO nc
484 LET sk = 0
486 FOR j = 1 TO up: IF u(j) = i THEN LET sk = 1: NEXT j
488 IF sk = 1 THEN GO TO 498
490 LET tg = 0
492 IF f(i) = s1 THEN LET tg = g(i)
494 IF g(i) = s1 THEN LET tg = f(i)
496 IF tg > 0 THEN LET a = tg: CIRCLE INK 3; x(a), y(a), 5
498 NEXT i
499 RETURN
500 REM === draw HUD ===
...
830 LET ci = 4: IF pz > 4 THEN LET ci = 5
832 LET st = 8
...
1245 LET uc = uc + 1
1250 BEEP 0.05, -5
1255 GO SUB 400: GO SUB 450: GO SUB 500
1260 RETURN
1300 REM === hint ===
1305 FOR i = 1 TO nc
1310 LET sk = 0
...
2557 PRINT AT 13, 3; INK 7; "H = hint"
2560 PRINT AT 15, 3; INK 5; "8 puzzles in 2 bands"
2570 PRINT AT 17, 3; INK 4; "Stars: * complete"
2575 PRINT AT 18, 3; INK 4; "       ** no undo"
2580 PRINT AT 19, 3; INK 4; "       *** clean solve"
