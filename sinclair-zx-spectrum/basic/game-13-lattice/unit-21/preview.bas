...
510 RETURN
600 REM === show preview lines from s1 ===
605 FOR i = 1 TO nc
610 REM Skip already drawn
615 LET sk = 0
620 FOR j = 1 TO up
625 IF u(j) = i THEN LET sk = 1
630 NEXT j
635 IF sk = 1 THEN GO TO 660
640 LET tg = 0
645 IF f(i) = s1 THEN LET tg = g(i)
650 IF g(i) = s1 THEN LET tg = f(i)
652 IF tg = 0 THEN GO TO 660
655 PLOT INK 3; x(s1), y(s1): DRAW INK 3; x(tg) - x(s1), y(tg) - y(s1)
660 NEXT i
665 RETURN
650 REM === erase preview lines from s1 ===
652 FOR i = 1 TO nc
654 LET sk = 0
...
670 PLOT INK 0; x(s1), y(s1): DRAW INK 0; x(tg) - x(s1), y(tg) - y(s1)
680 NEXT i
682 GO SUB 400
...
810 IF ik = 4 OR ik = 5 THEN LET cx = 1
815 NEXT j
