...
1752 LET p = u: LET q = v
1754 LET i = -1
1756 LET k = -1
1758 IF q + i * 2 < 1 OR q + i * 2 > 8 OR p + k * 2 < 1 OR p + k * 2 > 8 THEN GO TO 1770
1760 IF b(q + i * 2, p + k * 2) <> 0 THEN GO TO 1770
1762 IF b(q + i, p + k) = 0 THEN GO TO 1770
1764 IF t = 1 AND b(q + i, p + k) <> 2 AND b(q + i, p + k) <> 4 THEN GO TO 1770
1766 IF t = 2 AND b(q + i, p + k) <> 1 AND b(q + i, p + k) <> 3 THEN GO TO 1770
1767 IF b(q, p) < 3 AND t = 1 AND i < 0 THEN GO TO 1770
1768 IF b(q, p) < 3 AND t = 2 AND i > 0 THEN GO TO 1770
1769 PRINT AT 15, 1; INK 5; "Jump again!         ": GO SUB 800: GO TO 110
