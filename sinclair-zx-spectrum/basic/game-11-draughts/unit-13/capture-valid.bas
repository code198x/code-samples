...
800 REM === draw status ===
810 PRINT AT 12, 1; "                              "
820 IF t = 1 THEN PRINT AT 12, 1; INK 2; "Player 1 (Red)  "; g; "-"; h
830 IF t = 2 THEN PRINT AT 12, 1; INK 4; "Player 2 (Green)  "; g; "-"; h
840 RETURN
850 REM === read input ===
852 PRINT AT 14, 1; "                              "
...
970 IF e <> 1 OR f <> 1 THEN PRINT AT 14, 1; INK 2; "Invalid move        ": GO TO 854
975 IF b(v, u) <> 0 THEN PRINT AT 14, 1; INK 2; "Square occupied     ": GO TO 854
...
1020 LET b(q, p) = 0
1030 PRINT AT q + 2, p * 2 + 2; PAPER 0; " "
1040 IF t = 1 THEN PRINT AT v + 2, u * 2 + 2; PAPER 0; INK 2; "o"
1050 IF t = 2 THEN PRINT AT v + 2, u * 2 + 2; PAPER 0; INK 4; "o"
1055 IF j = 1 THEN LET w = (q + v) / 2: LET x = (p + u) / 2: LET b(w, x) = 0: PRINT AT w + 2, x * 2 + 2; PAPER 0; " ": BEEP 0.1, 5: IF t = 1 THEN LET g = g + 1: GO TO 1060
1056 IF j = 1 THEN LET h = h + 1
1060 BEEP 0.05, 10
1070 IF t = 1 THEN LET t = 2: RETURN
1080 LET t = 1
1090 RETURN
1100 REM === check capture ===
1110 IF b(v, u) <> 0 THEN PRINT AT 14, 1; INK 2; "Square occupied     ": GO TO 854
1120 LET w = (q + v) / 2: LET x = (p + u) / 2
1130 IF b(w, x) = 0 THEN PRINT AT 14, 1; INK 2; "Nothing to jump     ": GO TO 854
1140 IF t = 1 AND b(w, x) <> 2 THEN PRINT AT 14, 1; INK 2; "Can only jump enemy ": GO TO 854
1150 IF t = 2 AND b(w, x) <> 1 THEN PRINT AT 14, 1; INK 2; "Can only jump enemy ": GO TO 854
1160 IF t = 1 AND v < q THEN PRINT AT 14, 1; INK 2; "Must jump forward   ": GO TO 854
1170 IF t = 2 AND v > q THEN PRINT AT 14, 1; INK 2; "Must jump forward   ": GO TO 854
1180 LET j = 1
1190 RETURN
