...
856 IF LEN m$ < 5 THEN GO SUB 1900: PRINT AT 15, 1; INK 2; "Use format: A3 B4": GO TO 854
860 LET p = CODE m$(1) - 64
862 LET q = VAL m$(2 TO 2)
864 LET u = CODE m$(4) - 64
866 LET v = VAL m$(5 TO 5)
870 IF p < 1 OR p > 8 OR q < 1 OR q > 8 THEN GO SUB 1900: PRINT AT 15, 1; INK 2; "Invalid square": GO TO 854
872 IF u < 1 OR u > 8 OR v < 1 OR v > 8 THEN GO SUB 1900: PRINT AT 15, 1; INK 2; "Invalid square": GO TO 854
880 RETURN
900 REM === validate source ===
910 IF b(q, p) = 0 THEN GO SUB 1900: PRINT AT 15, 1; INK 2; "No piece there      ": GO TO 854
920 IF t = 1 AND b(q, p) <> 1 AND b(q, p) <> 3 THEN GO SUB 1900: PRINT AT 15, 1; INK 2; "Not your piece      ": GO TO 854
930 IF t = 2 AND b(q, p) <> 2 AND b(q, p) <> 4 THEN GO SUB 1900: PRINT AT 15, 1; INK 2; "Not your piece      ": GO TO 854
935 IF l = 1 AND (p <> y OR q <> s) THEN GO SUB 1900: PRINT AT 15, 1; INK 2; "Must jump with "; CHR$ (64 + y); s: GO TO 854
940 RETURN
950 REM === check move ===
955 LET j = 0
960 LET e = ABS (u - p): LET f = ABS (v - q)
962 IF e = 2 AND f = 2 THEN GO TO 1100
965 IF l = 1 THEN GO SUB 1900: PRINT AT 15, 1; INK 2; "Must capture        ": GO TO 854
970 IF e <> 1 OR f <> 1 THEN GO SUB 1900: PRINT AT 15, 1; INK 2; "Invalid move        ": GO TO 854
975 IF b(v, u) <> 0 THEN GO SUB 1900: PRINT AT 15, 1; INK 2; "Square occupied     ": GO TO 854
980 IF b(q, p) < 3 AND t = 1 AND v < q THEN GO SUB 1900: PRINT AT 15, 1; INK 2; "Must move forward   ": GO TO 854
990 IF b(q, p) < 3 AND t = 2 AND v > q THEN GO SUB 1900: PRINT AT 15, 1; INK 2; "Must move forward   ": GO TO 854
992 GO SUB 1600
993 IF z = 1 THEN GO SUB 1900: PRINT AT 15, 1; INK 2; "Capture with "; CHR$ (64 + y); s: GO TO 854
995 RETURN
1000 REM === make move ===
...
1110 IF b(v, u) <> 0 THEN GO SUB 1900: PRINT AT 15, 1; INK 2; "Square occupied     ": GO TO 854
1120 LET w = (q + v) / 2: LET x = (p + u) / 2
1130 IF b(w, x) = 0 THEN GO SUB 1900: PRINT AT 15, 1; INK 2; "Nothing to jump     ": GO TO 854
1140 IF t = 1 AND b(w, x) <> 2 AND b(w, x) <> 4 THEN GO SUB 1900: PRINT AT 15, 1; INK 2; "Can only jump enemy ": GO TO 854
1150 IF t = 2 AND b(w, x) <> 1 AND b(w, x) <> 3 THEN GO SUB 1900: PRINT AT 15, 1; INK 2; "Can only jump enemy ": GO TO 854
1160 IF b(q, p) < 3 AND t = 1 AND v < q THEN GO SUB 1900: PRINT AT 15, 1; INK 2; "Must jump forward   ": GO TO 854
1170 IF b(q, p) < 3 AND t = 2 AND v > q THEN GO SUB 1900: PRINT AT 15, 1; INK 2; "Must jump forward   ": GO TO 854
1180 LET j = 1
1190 RETURN
1200 REM === draw cell ===
