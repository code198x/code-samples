...
source=ROOT/'dice-roller.bas';rows=source.read_text().splitlines();numbers=[int(s.split()[0]) for s in rows];assert numbers==sorted(set(numbers))
...
tape=out/'dice-roller.tap';assert build['tape_sha256']==sha(tape)
...
manifest={'configuration':build['configuration'],'source_sha256':sha(source),'binary_sha256':build['binary_sha256'],'checks':len(results['checks']),'boundary_fixtures':len(results['fixtures']),'files':{p.name:sha(p) for p in out.iterdir() if p.is_file() and p.name!='manifest.json'},'limits':'Original key-driven emulator captures; read-only memory. Zero/100% bars and the 8400-roll starting total use labelled ordinary ROM setup commands; maximum.png is diagnostic. Native acceptance pending.'}
(out/'manifest.json').write_text(json.dumps(manifest,indent=2)+'\n');print('PASS',manifest['checks'],'execution groups;',manifest['boundary_fixtures'],'boundary fixtures; tape and source identities')
