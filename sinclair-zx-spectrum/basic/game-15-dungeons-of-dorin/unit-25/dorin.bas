...
114 REM monster count scales with floor
...
128 GO SUB 1700
130 NEXT i
132 REM chests: fewer on deep floors
134 FOR i = 1 TO 2
136 GO SUB 1600
138 LET d(t2,t) = 3
140 NEXT i
142 REM fountain: rarer on deep floors
144 IF e <= 5 AND RND > 0.4 THEN GO SUB 1600: LET d(t2,t) = 4
146 IF e > 5 AND RND > 0.7 THEN GO SUB 1600: LET d(t2,t) = 4
148 GO SUB 380
150 CLS
152 GO SUB 300
154 GO SUB 500
156 REM === main loop ===
158 PAUSE 0: LET i$ = INKEY$
160 LET b = b + 1
162 LET t = x: LET t2 = y
164 IF i$ = "q" OR i$ = "Q" THEN LET t2 = y - 1
166 IF i$ = "a" OR i$ = "A" THEN LET t2 = y + 1
168 IF i$ = "o" OR i$ = "O" THEN LET t = x - 1
170 IF i$ = "p" OR i$ = "P" THEN LET t = x + 1
172 IF i$ = "h" OR i$ = "H" THEN GO SUB 700: GO TO 156
174 IF i$ = "i" OR i$ = "I" THEN GO SUB 1800: GO TO 156
176 IF t = x AND t2 = y THEN GO TO 156
178 IF t < 1 OR t > 16 OR t2 < 1 OR t2 > 16 THEN GO TO 156
180 IF d(t2,t) = 1 THEN GO TO 156
182 FOR c = 1 TO m
184 IF j(c) = t AND k(c) = t2 AND n(c) > 0 THEN LET i = c: GO SUB 800: GO TO 198
186 NEXT c
188 LET x = t: LET y = t2
190 IF d(y,x) = 2 THEN GO SUB 600: GO TO 156
192 IF d(y,x) = 3 THEN GO SUB 750: LET d(y,x) = 0
194 IF d(y,x) = 4 THEN LET h = z: LET d(y,x) = 0: PRINT AT 12, 3; INK 5; "Fountain! HP restored!"
198 GO SUB 380
200 GO SUB 300
202 GO SUB 500
204 GO TO 156
...
710 PRINT AT 12, 5; INK 4; "Healed! HP:"; h; "  "
714 RETURN
750 REM === open chest ===
752 LET i = INT (RND * 4)
...
1824 PRINT AT 20, 4; INK 7; "Press any key"
1826 PAUSE 0
1828 CLS
1830 RETURN
