...
source=ROOT/'caverns.bas';rows=source.read_text().splitlines();numbers=[int(s.split()[0]) for s in rows];assert numbers==sorted(set(numbers))
...
tape=out/'caverns.tap';assert build['tape_sha256']==sha(tape)
