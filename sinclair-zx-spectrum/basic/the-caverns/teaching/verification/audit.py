...
for item in json.loads((ROOT/'checkpoints.json').read_text()):
 name=item['name'];folder=ROOT/name;source=folder/'caverns.bas';out=ROOT/'verification/evidence'/name
 ...
 data=(out/'caverns.tap').read_bytes();pos=0;blocks=[]
 while pos<len(data):
 ...
 build=json.loads((out/'build.json').read_text());result=json.loads((out/'results.json').read_text());assert build['source_sha256']==result['source_sha256']==sha(source);assert build['tape_sha256']==sha(out/'caverns.tap');assert result['direct_memory_writes']==False
 reports.append(dict(name=name,source_sha256=sha(source),tape_sha256=sha(out/'caverns.tap'),checks=result['checks'],captures={p.name:sha(p) for p in sorted(out.glob('*.png'))},server=build['server'],binary_sha256=build['binary_sha256']))
