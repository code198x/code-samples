...
The builder reuses the maintained Meet BASIC keyboard harness. Load the resulting `drift.tap` with `LOAD ""`. Native playability and teaching stages are pending review.
...
At rest, 41 complete movement-commit intervals had a median of 19 PAL frames and range 18–19: about 2.6 updates per second. This is not a fixed flight rate or host-input latency measurement. The state-model checks use instruction stepping; their session clock is not a timing benchmark. Native playability and original-hardware performance remain unverified.
