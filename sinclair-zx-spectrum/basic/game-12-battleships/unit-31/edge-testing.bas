...
40 DIM h(2, 5): DIM k(2, 5)
42 LET s1 = 0: LET s2 = 0
50 LET p = 1
55 CLS
60 GO SUB 300
70 CLS
...
331 IF m$ = "" THEN PRINT AT 16, 1; INK 2; "Enter a coordinate": GO TO 330
332 IF LEN m$ < 2 THEN PRINT AT 16, 1; INK 2; "Use format: A3": GO TO 330
333 IF LEN m$ > 3 THEN PRINT AT 16, 1; INK 2; "Use format: A3": GO TO 330
335 LET x = CODE m$(1) - 64
336 IF x > 26 THEN LET x = x - 32
337 IF x < 1 OR x > 10 THEN PRINT AT 16, 1; INK 2; "Column A-J only": GO TO 330
338 IF LEN m$ = 2 THEN LET y = VAL m$(2)
339 IF LEN m$ > 2 THEN LET y = VAL m$(2 TO LEN m$)
340 IF y < 1 OR y > 10 THEN PRINT AT 16, 1; INK 2; "Row 1-10 only": GO TO 330
345 INPUT "Direction (H/V): "; d$
346 IF d$ = "" THEN PRINT AT 16, 1; INK 2; "Enter H or V": GO TO 345
...
399 NEXT s
401 GO SUB 600
402 PRINT AT 14, 1; INK 7; "Happy with layout? (Y/N)"
403 PAUSE 0
404 LET r$ = INKEY$
405 IF r$ = "n" OR r$ = "N" THEN GO SUB 410: GO TO 305
406 RETURN
410 REM clear player grid
412 FOR r = 1 TO 10
414 FOR q = 1 TO 10
416 IF p = 1 THEN LET a(r, q) = 0
418 IF p = 2 THEN LET b(r, q) = 0
420 NEXT q
422 NEXT r
424 CLS
426 RETURN
...
520 NEXT n
522 PRINT AT 1, 12; INK 5; "|"
523 PRINT AT 1, 29; INK 5; "|"
525 FOR r = 1 TO 10
...
575 NEXT q
576 PRINT AT r + 1, 12; INK 5; "|"
577 PRINT AT r + 1, 29; INK 5; "|"
580 NEXT r
582 FOR n = 0 TO 12
583 PRINT AT 12, n; INK 5; "-"
584 PRINT AT 12, n + 17; INK 5; "-"
585 NEXT n
586 RETURN
...
660 RETURN
700 REM === draw status ===
710 PRINT AT 13, 1; INK 6; BRIGHT 1; "Player "; p
715 PRINT AT 14, 1; INK 7; "Shots: ";
717 IF p = 1 THEN PRINT s1; "  "
719 IF p = 2 THEN PRINT s2; "  "
720 RETURN
800 REM === fire shot ===
805 PRINT AT 16, 1; "                              "
806 PRINT AT 17, 1; "                              "
810 INPUT "Target (e.g. D7): "; m$
811 IF m$ = "" THEN PRINT AT 17, 1; INK 2; "Enter a coordinate": GO TO 810
812 IF LEN m$ < 2 THEN PRINT AT 17, 1; INK 2; "Use format: D7": GO TO 810
813 IF LEN m$ > 3 THEN PRINT AT 17, 1; INK 2; "Use format: D7": GO TO 810
815 LET x = CODE m$(1) - 64
816 IF x > 26 THEN LET x = x - 32
817 IF x < 1 OR x > 10 THEN PRINT AT 17, 1; INK 2; "Column A-J only": GO TO 810
818 IF LEN m$ = 2 THEN LET y = VAL m$(2)
819 IF LEN m$ > 2 THEN LET y = VAL m$(2 TO LEN m$)
820 IF y < 1 OR y > 10 THEN PRINT AT 17, 1; INK 2; "Row 1-10 only": GO TO 810
825 LET f = 0
827 IF p = 1 AND c(y, x) <> 0 THEN LET f = 1
829 IF p = 2 AND d(y, x) <> 0 THEN LET f = 1
830 IF f = 1 THEN PRINT AT 17, 1; INK 2; "Already fired there": GO TO 810
835 IF p = 1 THEN LET s1 = s1 + 1
837 IF p = 2 THEN LET s2 = s2 + 1
840 RETURN
...
926 LET o = 1: IF p = 1 THEN LET o = 2
928 LET h(o, v) = h(o, v) + 1
930 PRINT AT 18, 1; INK 2; BRIGHT 1; "HIT!                          "
931 PRINT AT y + 1, x + 18; FLASH 1; INK 2; BRIGHT 1; "X"
932 BEEP 0.1, 15: BEEP 0.05, 20
933 PRINT AT y + 1, x + 18; FLASH 0; INK 2; BRIGHT 1; "X"
934 IF h(o, v) = l(v) THEN LET k(o, v) = 1: PRINT AT 19, 1; INK 2; BRIGHT 1; "You sank the "; n$(v); "!": BEEP 0.1, 12: BEEP 0.1, 8: BEEP 0.1, 4
936 GO TO 960
950 REM miss
952 IF p = 1 THEN LET c(y, x) = 1
954 IF p = 2 THEN LET d(y, x) = 1
956 PRINT AT 18, 1; INK 6; "Miss.                         "
958 BEEP 0.1, -5
960 GO SUB 500
965 RETURN
1000 REM === check win ===
1005 LET o = 1: IF p = 1 THEN LET o = 2
1010 LET w = 0
1020 FOR n = 1 TO 5
1030 IF k(o, n) = 1 THEN LET w = w + 1
1040 NEXT n
1050 IF w < 5 THEN RETURN
1060 PRINT AT 18, 1; FLASH 1; INK 7; BRIGHT 1; "Player "; p; " WINS!             "
1065 PRINT AT 19, 1; INK 7; "P1: "; s1; " shots  P2: "; s2; " shots"
1070 BEEP 0.2, 12: BEEP 0.2, 16: BEEP 0.3, 19
1075 PRINT AT 20, 2; INK 7; "Play again? (Y/N)"
1080 PAUSE 0
1085 LET r$ = INKEY$
1090 IF r$ = "y" OR r$ = "Y" THEN GO TO 10
1095 STOP
1100 REM === ships remaining ===
1105 LET o = 1: IF p = 1 THEN LET o = 2
1110 PRINT AT 20, 1; INK 7; "Enemy: ";
1120 FOR n = 1 TO 5
1130 IF k(o, n) = 1 THEN PRINT INK 2; CHR$ (64 + n); " ";
1140 IF k(o, n) = 0 THEN PRINT INK 4; CHR$ (64 + n); " ";
1150 NEXT n
1160 RETURN
