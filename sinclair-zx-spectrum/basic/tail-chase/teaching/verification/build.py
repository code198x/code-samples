...
def build(item,exe,output):
 ...
 try:
  ...
  tape=out/'tail.tap';m.call('save_tape',path=str(tape))
  (out/'stored.json').write_text(json.dumps(stored)+'\n')
  record={'name':item['name'],'source':item['source'],'source_sha256':sha(source),'tape_sha256':sha(tape),'binary_sha256':sha(Path(exe)),'server':m.server,'entry':'Fresh 48K ROM keyboard; SAVE "tail" LINE 10; no injection'}
