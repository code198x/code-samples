...
try:
 m.load_source(ROOT/'brick-bash.bas');stored=m.program_lines()
 m.statement('SAVE "bricks" LINE 10');m.enter()
 for _ in range(1500):
  if any('0 OK' in s for s in m.screen()):break
  m.frames(20)
 else:raise AssertionError(m.screen())
 m.call('save_tape',path=str((a.output/'bricks.tap').resolve()))
 (a.output/'stored.json').write_text(json.dumps(stored)+'\n')
 (a.output/'build.json').write_text(json.dumps({'source_sha256':hashlib.sha256((ROOT/'brick-bash.bas').read_bytes()).hexdigest(),'tape_sha256':hashlib.sha256((a.output/'bricks.tap').read_bytes()).hexdigest(),'entry':'ROM keyboard; SAVE "bricks" LINE 10; no source injection'},indent=2)+'\n')
finally:m.close()
