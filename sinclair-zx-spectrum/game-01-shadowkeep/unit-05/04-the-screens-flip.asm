; ============================================================================
; SHADOWKEEP — Unit 5, Stage 4: The Screens Flip
; ============================================================================
; The doors are decorative until they do something. This stage gives them
; their job.
;
; When `try_move` reads `DOOR_OPEN` at the target cell, it doesn't perform
; the normal restore → update → draw cycle. It tail-calls `do_transition`,
; which flips `current_room`, clears the screen, redraws the new room from
; scratch, and places the hero at the *arrival* cell — one cell *past* the
; destination's door, inside the destination room. The hero never lingers
; on a door cell; he passes through it.
;
; The arrival points are hardcoded against the door positions:
;
;   Great hall east door  (col 26, row 10)  →  Antechamber arrival (col 6, row 10)
;   Antechamber west door (col 5,  row 10)  →  Great hall  arrival (col 25, row 10)
;
; A more elaborate keep would store these in a "door pairs" table indexed
; by (from_room, door_col, door_row). For two rooms, hardcoding is fine —
; the table arrives in Phase 2 when there are nine rooms to connect.
; ============================================================================
...
; ----------------------------------------------------------------------------
; clear_screen: wipe bitmap to 0 and attributes to dark stone.
; ----------------------------------------------------------------------------
...
; ----------------------------------------------------------------------------
; draw_current_room: render the room selected by `current_room`.
; ----------------------------------------------------------------------------
...
; ----------------------------------------------------------------------------
; try_move: walk normally for FLOOR/GOLD; transition for DOOR_OPEN; block else.
; ----------------------------------------------------------------------------
...
; ----------------------------------------------------------------------------
; do_transition: flip current_room, wipe screen, redraw, deposit hero at
; the arrival cell in the destination room.
; ----------------------------------------------------------------------------
...
; ----------------------------------------------------------------------------
; Address helpers, save/restore/draw — same as Stage 3
; ----------------------------------------------------------------------------
