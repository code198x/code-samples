;
; Shadowkeep Unit 8, Stage 4 — The Keep Stands
;
; Stage 3 ran title → gameplay → halt. This stage closes the loop:
; title → gameplay → victory → title again. The keep becomes a complete
; experience the player can play through repeatedly.
;
; Two changes from Stage 3:
;
;   1. Room data is restored before every playthrough. Stage 3's gold
;      pickup mutated `room_data_great_hall` and `room_data_antechamber`
;      in place. A second playthrough would find an empty keep. So we
;      keep the source data as a constant byte table (`_src` labels)
;      and LDIR-copy it into a RAM workspace (`_rw`) every time the
;      game starts. `room_table` points at the workspace.
;
;   2. Victory routes back to the title. After the green-BORDER
;      celebration and a brief held chord, `jp title_loop` re-enters
;      the presentation, which plays the music until SPACE starts the
;      next playthrough. The keep is winnable; the keep is replayable.
;
...
title_loop:
            ...
            ; Wait briefly for SPACE release so it doesn't re-trigger.
            ld      b, 8
.space_settle:
            halt
            djnz    .space_settle

            ; Reset all game state and render the starting room.
            call    init_state
            call    init_game
            jp      main_loop

; ----------------------------------------------------------------------------
; init_state: copy the source room data into the working buffers, so a
; new playthrough starts with all gold cells restored. 352 bytes per
; room × 2 rooms = 704 bytes total. LDIR handles each in one instruction.
; ----------------------------------------------------------------------------
...
victory:
            ld      a, 4                ; green BORDER — the keep stands
            out     ($FE), a

            ; A held victory note — D5 for about half a second.
            ld      a, 112              ; D6 (high-pitched cheer)
            ld      de, 250
            call    play_note
            ...
            ; Pause for a few frames so the green BORDER is seen.
            ld      b, 60
.hold:      halt
            djnz    .hold

            ld      a, 0
            out     ($FE), a            ; BORDER back to black
            jp      title_loop          ; return to title — the keep is replayable
...
; room_table points at the RW workspaces, which init_state populates
; from *_src each time a playthrough begins.
room_table:
...
; RAM workspaces — uninitialised at load (the SNA leaves them zero).
; init_state copies *_src into these before each playthrough.
room_data_great_hall:
            defs    352
room_data_antechamber:
            defs    352

; Source data — constant byte tables. Never written to during play.
room_data_great_hall_src:
