...
wait_zero:
            ...
            ; Handle keyboard input (same as Unit 1)
            jsr check_x_key
            cmp #$01
            beq ml_x_pressed
...
move_loop:
            ...
            ; Check if off screen (underflow to $FF or at 0)
            lda note_x,x
            cmp #NOTE_INACTIVE     ; Did it wrap to $FF?
            beq despawn_note
            cmp #$00               ; At column 0?
            beq move_draw          ; Draw at 0, despawn next frame
...
despawn_note:
            ; Mark as inactive (already $FF from decrement)
            jmp move_next
...
erase_note:
            ...
            lda note_x,x
            cmp #HIT_ZONE_X        ; In hit zone?
            bcc erase_note_done    ; Yes - don't erase hit zone
