...
; Game over/song end text positions
GAMEOVER_ROW    = 11
GAMEOVER_COL    = 14
SONGEND_ROW     = 11
SONGEND_COL     = 12
...
ml_continue1:
            ...
            ; Handle song-driven spawning (replaces fixed timer)
            jsr check_song
...
;───────────────────────────────────────
; Trigger song complete - NEW for Unit 7
;───────────────────────────────────────
trigger_song_complete:
            lda #$00
            sta game_running

            ldx #$00
tsc_loop:
            lda songcomplete_text,x
            beq tsc_done
            sta SCREEN + (SONGEND_ROW * 40) + SONGEND_COL,x
            lda #COL_GREEN
            sta COLOUR + (SONGEND_ROW * 40) + SONGEND_COL,x
            inx
            bne tsc_loop
tsc_done:
            rts
...
move_next:
            ...
            ; After moving all notes, check if song ended and no notes left
            lda song_playing
            bne mn_done
            jsr check_notes_remaining
mn_done:
...
check_c_key:
            lda #%11111011              ; Row 2 (C is at row 2, column 4)
            sta CIA1_PRA
            lda CIA1_PRB
...
check_v_key:
            lda #%11110111              ; Row 3 (V is at row 3, column 7)
            sta CIA1_PRA
            lda CIA1_PRB
...
; Song playback variables - NEW for Unit 7
next_note_timer: !byte $00
song_playing:   !byte $00

; Zero page pointers for indirect addressing
screen_ptr      = $fb
...
;───────────────────────────────────────
; Song Data - NEW for Unit 7
; Format: delta_frames, track (0-2)
; delta = frames to wait before spawning this note
; track = 0 (top/X), 1 (middle/C), 2 (bottom/V)
; End marker: 0, 0
;───────────────────────────────────────
song_data:
            ; Intro - single notes to get started
