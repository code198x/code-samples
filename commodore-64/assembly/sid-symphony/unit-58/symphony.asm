; ============================================================================
; SID SYMPHONY - Unit 58: Two Player Mode
; ============================================================================
; This unit adds two-player competitive mode - two players compete on the
; same song, each with their own score, health, and combo. Player 2 uses
; keys 1, 2, 3 for their three tracks.
;
; CONCEPTS COVERED:
; - Duplicate game state for second player
; - Shared keyboard handling (split keyboard)
; - Split screen display
; - Competitive scoring
;
; OUR APPROACH:
; - Player 1: Z, X, C keys (left side of keyboard)
; - Player 2: 1, 2, 3 keys (top row)
; - Both players share same song/notes
; - Independent health and scoring
;
; Controls: P1: Z/X/C = Tracks | P2: 1/2/3 = Tracks
...
; Unit 58: Two Player Mode Variables
two_player_mode  = $4F          ; 0 = single player, 1 = two player
p2_hit_quality   = $55          ; Player 2 hit quality (like hit_quality)
...
start:
            ...
            ; Practice mode off, two player off, normal speed
            lda #0
            sta practice_mode
            sta two_player_mode     ; Unit 58: Single player default
...
draw_endless_label_done:
            ...
            jsr draw_2p_state       ; Unit 58: Two player mode display
            rts
...
; ----------------------------------------------------------------------------
; Draw Two Player Mode State (Unit 58)
; Display "2 PLAYER: ON/OFF" on line 24 (left side)
; ----------------------------------------------------------------------------

draw_2p_state:
            ; Display "2P:ON/OFF" at row 20, column 32 (right side, below songs)
            ldx #0
twop_label_loop:
            lda twoplayer_label,x
            beq twop_draw_value
            sta SCREEN + (20 * 40) + 32,x
            lda #TITLE_COL
            sta COLRAM + (20 * 40) + 32,x
            inx
            jmp twop_label_loop
...
menu_check_keys:
            ...
            lda CIA1_PRB
            and #$08                ; Column 3 = -
            bne menu_not_minus      ; Branch around far jump
            jmp menu_minus_pressed
menu_not_minus:
            ...
            ; Check 2 key for two player mode (row 3, column 0) - Unit 58
            lda #$F7                ; Row 3
            sta CIA1_PRA
            lda CIA1_PRB
            and #$01                ; Column 0 = 2
